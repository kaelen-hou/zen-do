...
只返回JSON，不要其他文字。`;
  ...
  private static validateAndCleanParsedTask(parsed: Record<string, unknown>): ParsedTask {
    // 验证必填字段
    ...
    const priority: Priority = validPriorities.includes(parsed.priority)
      ? parsed.priority
      : 'medium';
    ...
    return {
      ...
      description: parsed.description && typeof parsed.description === 'string'
